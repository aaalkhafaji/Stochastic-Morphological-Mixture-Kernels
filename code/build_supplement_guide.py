...
def main():
    ...
    p('Reproducibility guide | Strengthening branch Step 9 | Baseline archive v2.1.0 | Prepared 8 September 2026','SmallS')
    ...
    p('The original campaign evaluates a fixed morphological bank on a held-out test set. The subsequent output-space construction reuses that inspected set and is explicitly exploratory. Steps 5 and 6 add independent public-data validations: Oxford-IIIT Pet supplies higher-resolution human annotations under frozen synthetic corruptions, while DAVIS 2016 supplies genuine algorithm-produced segmentation errors with no synthetic corruption. Step 7 then freezes the Step-5 IID-fitted selectors and probes nine structured Oxford error families without refitting or retuning. Step 8 adds a validation-only fair-baseline audit against public SoftMorph2, an empirical 3x3 W-operator, and an intentionally advantaged condition-aware fixed-bank control. Step 9 closes the theory--computation loop with finite stress tests of replication sensitivity, topology sharpness, incidence rank, Bayesian reversal, and kernel composition.')
    ...
    if (ROOT/'results/step7_structured/step7_summary.csv').exists():
        ...
        p('Step 8 adds matched modern/fair morphology controls. Public SoftMorph2 is retrieved at runtime and independently verified at maximum discrepancy 0.0; a regularized empirical 3x3 W-operator is tuned on validation only. On IID Oxford, both learned selectors significantly beat SoftMorph2 and the W-operator, while the condition-aware fixed control remains descriptively strongest. On structured OOD, direct-output loss is 0.10104 versus 0.15195 for SoftMorph2 and 0.16448 for the W-operator; SoftMorph2 significantly beats action-coordinate selection, and all directions are retained under the eight-comparison family-wise plan.')
    ...
    p('Individual metrics do not all improve: MNIST Dice and shapes hole error can worsen. Stronger Fashion-MNIST corruption gives a documented failure against the fixed area filter. Clean inputs can change. Evaluating a full 31-action bank is materially slower than using a single area filter. Oxford closes the low-resolution-only gap, DAVIS closes the synthetic-error-only gap, Step 7 probes structured mask-error shift, Step 8 adds a matched SoftMorph2/W-operator comparison, and Step 9 stress-tests theorem consequences on small finite state spaces; none establishes RGB restoration, clinical performance, correlated real-sensor-noise robustness, or native-resolution topology preservation. DAVIS source segmentations are from the 2016 benchmark rather than contemporary architectures.')
